...
from essentials_item import (  # noqa: E402
    COMPILED_ITEM_FILE,
    ITEM_MESSAGES_FILE,
    ITEM_PBS_FILE,
...
def _arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validation privee bornee d'une Item.Description v21.1."
    )
...
def _publish_project(
    ...
    report_payload = (
        "PFT v21.1 - validation privee Item.Description\n"
...
def main() -> int:
    ...
    item_rows = [row for row in rows if row["fichier"] == ITEM_PBS_FILE]
    allowed_text_fields = {
        "Name", "NamePlural", "PortionName", "PortionNamePlural", "Description"
    }
    technical_rows = [
        row for row in item_rows if row["commande"] not in allowed_text_fields
    ]
    if technical_rows:
        raise RuntimeError("Un champ technique de items.txt a ete expose comme texte.")
    description_rows = [row for row in item_rows if row["commande"] == "Description"]
    selected = next(
    ...
    selected["origine_traduction"] = "validation_privee_v21_1_item_description"
    ...
    plan = build_v21_1_item_description_validation_plan(work, csv_path)
    simulate_plan(plan)
    ...
    reextracted = extract_item_description_texts(
        (candidate / ITEM_PBS_FILE).read_bytes(),
        (candidate / COMPILED_ITEM_FILE).read_bytes(),
        (candidate / ITEM_MESSAGES_FILE).read_bytes(),
    ...
    proofs = build_item_text_proofs(
        (work / ITEM_PBS_FILE).read_bytes(),
        (work / COMPILED_ITEM_FILE).read_bytes(),
        (work / ITEM_MESSAGES_FILE).read_bytes(),
    )
    ...
    if set(result.modified_files) != expected_files:
        raise RuntimeError("La liste des fichiers modifies ne correspond pas au plan.")
        ...
        json.dumps(
            {
                ...
                "item_description_rows": len(description_rows),
                "item_technical_rows": len(technical_rows),
                "item_description_unique_safe": safe_descriptions,
