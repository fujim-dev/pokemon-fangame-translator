...
from essentials_ability import (  # noqa: E402
...
def _arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validation privée bornée d'une Ability.Description v21.1."
    )
...
def _publish_project(
    ...
    report_payload = (
        "PFT v21.1 — validation privée Ability.Description\n"
...
def main() -> int:
    ...
    ability_rows = [
        ...
        and row["commande"] == "Description"
    ]
    selected = next(
        row for row in ability_rows if row["evenement_id"] == args.section
    )
    selected["traduction_fr"] = selected["texte_source"] + args.marker
    selected["statut"] = "Accepté"
    selected["origine_traduction"] = "validation_privee_v21_1_ability_description"
    ...
    plan = build_v21_1_ability_description_validation_plan(work, csv_path)
    simulate_plan(plan)
    ...
    reextracted = extract_ability_description_texts(
        (candidate / ABILITY_PBS_FILE).read_bytes(),
        (candidate / COMPILED_ABILITY_FILE).read_bytes(),
        (candidate / ABILITY_MESSAGES_FILE).read_bytes(),
    ...
    proofs = build_ability_description_proofs(
        (work / ABILITY_PBS_FILE).read_bytes(),
        (work / COMPILED_ABILITY_FILE).read_bytes(),
        (work / ABILITY_MESSAGES_FILE).read_bytes(),
    )
    safe_unique = sum(
        ...
        json.dumps(
            {
                ...
                "total_rows": len(rows),
                "ability_description_rows": len(ability_rows),
                "ability_description_structural_occurrences": len(proofs),
                "ability_description_unique_safe": safe_unique,
