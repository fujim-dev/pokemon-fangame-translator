...
from essentials_phone import (  # noqa: E402
    COMPILED_PHONE_FILE,
    PHONE_MESSAGES_FILE,
    PHONE_PBS_FILE,
...
def _arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validation privée bornée d'un message téléphone Essentials v21.1."
    )
    ...
    parser.add_argument("--key", default="End", choices=("End",))
...
def _publish_project(
    ...
    report_payload = (
        "PFT v21.1 — validation privée téléphone\n"
...
def main() -> int:
    ...
    phone_rows = [row for row in rows if row["fichier"] == PHONE_PBS_FILE]
    selected = next(
        row
        for row in phone_rows
        if row["evenement_id"] == args.section and row["commande"] == args.key
    )
    selected["traduction_fr"] = selected["texte_source"] + args.marker
    selected["statut"] = "Accepté"
    selected["origine_traduction"] = "validation_privee_v21_1_phone"
    ...
    plan = build_v21_1_phone_validation_plan(work, csv_path)
    simulate_plan(plan)
    ...
    reextracted = extract_phone_target_texts(
        (candidate / PHONE_PBS_FILE).read_bytes(),
        (candidate / COMPILED_PHONE_FILE).read_bytes(),
        (candidate / PHONE_MESSAGES_FILE).read_bytes(),
        section=selected["evenement_id"],
        key=selected["commande"],
    ...
    candidate_comparison = compare_snapshots(
        work_after,
        candidate_after,
        allowed_changed={PHONE_PBS_FILE, COMPILED_PHONE_FILE, PHONE_MESSAGES_FILE},
    )
        ...
        json.dumps(
            {
                ...
                "selected_key": selected["commande"],
